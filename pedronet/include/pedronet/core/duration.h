...
struct Duration : public Comparable<Duration> {
  ...
  Duration(std::chrono::duration<Rep, Period> other)
      : usecs(std::chrono::duration_cast<std::chrono::microseconds>(other)
                  .count()) {}
  Duration(int64_t usecs) : usecs(usecs) {}
  ...
  static Duration Seconds(int32_t secs) { return {secs * kMicroseconds}; }

  static Duration Milliseconds(int64_t ms) { return {ms * kMilliseconds}; }

  static Duration Microseconds(int64_t us) { return {us}; }
